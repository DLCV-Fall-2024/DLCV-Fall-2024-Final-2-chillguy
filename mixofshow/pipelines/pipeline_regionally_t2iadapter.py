...
class RegionallyT2IAdapterPipeline(StableDiffusionAdapterPipeline):
    ...
    def __call__(
        ...
        #print('keypose_input = ', keypose_input.shape) # torch.Size([1, 3, height, weight])
        ...
        #if(keypose_input is None and sketch_input is None):
        #    guidance_scale = 1.0
        ...
        #print('keypose_adapter_state = ', len(keypose_adapter_state), keypose_adapter_state[0].shape) # 4 x torch.Size()
        #print('sketch_adapter_state= ', sketch_adapter_state.shape)
        
        if(keypose_adapter_state is None and sketch_adapter_state is None):
            num_states = 1
        else:
            num_states = len(keypose_adapter_state) if keypose_adapter_state is not None else len(sketch_adapter_state)
        #print("num_states = ", num_states) # 4
        adapter_state = []
        ...
        for idx in range(num_states):
            if keypose_adapter_state is not None:
                ...
                if region_keypose_adaptor_weight != '':
                    ...
                    print('region_keypose_adaptor_weight != None')
                feat_keypose = spatial_adaptor_weight * feat_keypose
                print('feat_keypose =', feat_keypose.shape) # 
            else:
                feat_keypose = 0
        ...
        #print('adapter_state = ', len(adapter_state), adapter_state[0].shape) # 4 x tensor
        #if do_classifier_free_guidance:
        #    for k, v in enumerate(adapter_state):
        #        adapter_state[k] = torch.cat([v] * 2, dim=0)


        #torch.save(latents, 'latent_keypose.pt')
        #latents = torch.load('latent_keypose.pt')
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                #latent_model_input = torch.load('latent_model_input.pt')
                
                # print('latent_model_input = ', latent_model_input.shape) # [2, 4, 128, 256] -> [1, 4, 128, 256]
                # print('prompt_embeds = ', prompt_embeds.shape) # prompt_embeds =  torch.Size([2, 16, 77, 768]) -> torch.Size([2, 16, 77, 768])
                # print('region_list = ', len(region_list), len(region_list[0]), region_list[0][0].shape) # 3 x 2 x torch.Size([2, 16, 77, 768])
                # print('height = ', height)
                # print('width = ', width)
                ...
                noise_pred = self.unet(
                    ...
                    down_block_additional_residuals=[state for state in adapter_state],
                ).sample
